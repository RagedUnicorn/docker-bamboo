...
# @description launch script for bamboo
...
function database_check {
  if psql -h "${POSTGRESQL_DATABASE_HOSTNAME}" -U "${postgresql_app_user}" -w -lqt | cut -d \| -f 1 | grep -qw "${BAMBOO_DATABASE_NAME}"; then
    echo "$(date) [INFO]: Bamboo database already exists"
  else
    echo "$(date) [INFO]: Creating Bamboo database"
    createdb -h "${POSTGRESQL_DATABASE_HOSTNAME}" -U "${postgresql_app_user}"  -w "${BAMBOO_DATABASE_NAME}"
  fi
}
...
function init {
  ...
  echo "$(date) [INFO]: Starting Bamboo..."
